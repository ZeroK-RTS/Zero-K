	...
	for unitDefName, airjets in pairs(airjets_effects) do
		...
		for i, airjet in ipairs(airjets) do 
			...
			if not alreadyhasjet then
				...
				local airjetConfig = Spring.Utilities.CopyTable(longAirJet)
	...
	for i, target in pairs(targets) do 
		if UnitDefNames[source] and UnitDefNames[target] then 
			if unitDistortions[source]  then 
				unitDistortions[target] = Spring.Utilities.CopyTable(unitDistortions[source])
			end

			for eventName, distortions in pairs(unitEventDistortionsNames) do
				if unitEventDistortionsNames[eventName][source] then
					unitEventDistortionsNames[eventName][target] = Spring.Utilities.CopyTable(unitEventDistortionsNames[eventName][source])
				end
			end
		end
	end
...
for colorname, colorvalues in pairs(crystalColors) do
	for size = 1,3 do
		...
		if FeatureDefNames[crystaldefname] then
			local crystalDistortion = Spring.Utilities.CopyTable(crystalDistortionBase)
