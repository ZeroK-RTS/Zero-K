...
local function DrawBallHealthbar()
	for teamID, teamBalls in pairs(shieldBalls) do
		for _, shieldBall in pairs(teamBalls) do
			...
			glRotate(-90, 1, 0, 0)
			local shieldHpText = tostring(math.floor(shieldBall.currShield)) .. " / " .. tostring(shieldBall.totShield)
			font:Begin()
...
local function updateClustering()
	for _, teamID in pairs(Spring.GetTeamList()) do
		...
		opticsObject:Run()
		shieldBallsIdsByTeam[teamID] = opticsObject:Clusterize(700)
		allShieldUnitsByTeam[teamID] = allShieldUnits
	end
end
