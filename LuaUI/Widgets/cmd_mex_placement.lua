	...
	if -mexDefID == cmdID and WG.metalSpots then
		...
		if closestSpot then
			...
			if foundUnit then
				...
				if build ~= 1 then
					spGiveOrder(MD.REPAIR, {foundUnit}, options.coded)
				end
				return true
			else
...
function widget:Initialize() 
	local team = spGetMyTeamID()
	local units = spGetTeamUnits(team)
	for i, unitID in ipairs(units) do 
		widget:UnitCreated(unitID, spGetUnitDefID(unitID),team)
	end
end
...
function widget:DrawWorld()
	...
	if WG.metalSpots then
		...
		gl.LineWidth(1.49)
		gl.Color(1, 1, 0, 0.5)
		gl.BeginEnd(GL.LINE_STRIP, DoLine, bx, by, bz, closestSpot.x, closestSpot.y, closestSpot.z)
		gl.LineWidth(1.0)
		...
		gl.Color(1, 1, 1, 0.5)
		gl.PushMatrix()
		gl.Translate(closestSpot.x, closestSpot.y, closestSpot.z)
