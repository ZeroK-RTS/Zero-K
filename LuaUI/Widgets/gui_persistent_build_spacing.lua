function widget:GetInfo()
  ...
  return {
    name      = "Persistent Build Spacing 2",
...
options = {
	...
	show_time_rects = {
		name 			= 	'...within time before vanish.',
		type 			= 	'number',
		min 			= 	0.1,
		max 			= 	5,
		step 			= 	0.1,
		...
		OnChange 		= 	function(self)
								...
								if str=='forever' then self.name = "...forever."
								else self.name = '...within '..str..' before vanish.'
								end
								requestUpdate=options_path
							end,
		parents 		= 	{'show_spacing_rects'}
	...
	show_spacing_value = {
		...
		noHotkey 		= 	true,
		OnChange 		= 	function(self) -- masking useless options if any
...
function widget:Update(dt)
	if requestUpdate then UpdateOptionsDisplay(requestUpdate) requestUpdate=false end

    cmdID = select(2,spGetActiveCommand())
    if not cmdID or cmdID>=0 then time=0 draw=false return end
	...
	-- Drawing set up
	drawRects,drawValue=true,true

	if not showSpacingRects
	   or time>showRectsTime
	   or showRectsOnChange and newCmdID
	   then
	   drawRects=false
	end
	if not showSpacingValue
	   or time>showValueTime
	   or showValueOnChange and  newCmdID
	   then
	   drawValue=false
	end
	...
	if x==nx and z==nz and not(newspacing or newCmdID) then return end -- keeping already valid datas
	...
	local count=0
	if showSpacingRects and time<showRectsTime then
