...
local TIMER_DEFEAT = tonumber(modOptions.ctf_death_time or 120)
...
function widget:Update(s)
	...
	if timer > UPDATE_FREQUENCY then
		timer = 0
		UpdateTeamData()
		if (myTeam == nil) or (myAllyTeam == nil) or (RedAllyTeam == nil) then 
			return 
		end
		...
		if (blue_sd ~= nil) and (bf_stolen == 0) then
			if (blue_defeat == nil) and (bs == 0) then
				...
				if (blue_sd + 6 > TIMER_DEFEAT or blue_sd < 10) and blue_sd > 0  then
		...
		if (red_sd ~= nil) and (rf_stolen == 0)then
			if (red_defeat == nil) and (rs == 0) then
				...
				if (red_sd + 6 > TIMER_DEFEAT or red_sd < 10) and red_sd > 0 then
...
function widget:DrawScreen()
	...
	elseif otherText then
		...
		if (fh) then
			fh = fontHandler.UseFont(hudfont)
			fontHandler.DrawCentered(otherText)
		else
			glText(otherText, 0, 0, 20, "oc")
		end
		glPopMatrix()
	end
end
...
function widget:Initialize()
	local ctf = (tonumber(modOptions.ctf_enabled) == 1)
