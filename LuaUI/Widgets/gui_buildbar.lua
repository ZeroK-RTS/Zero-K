...
local repIcoSize = floor(iconSizeY*0.4)   --repeat iconsize
local fontSize   = iconSizeY * 0.35
local borderSize = 0.1
local maxVisibleBuilds = 4
...
local function UpdateIconSizes()
  ...
  fontSize  = iconSizeY * 0.35
  repIcoSize = floor(iconSizeY*0.4)
end
...
function widget:SetConfigData(data)
  ...
  bar_iconSizeBase = data.iconSizeBase or 40
...
local function DrawButton(rect, unitDefID, options)
  ...
  if options['repeat'] then
    DrawTexRect({rect[3]-repIcoSize,rect[2],rect[3],rect[2]-repIcoSize}, 'LuaUI/Images/repeat.png', 0.55)
  end
  ...
  elseif (options.pressed) then
    DrawRect(rect, { 0, 0, 0, 0.45 })  -- pressed
  ...
  if (options.waypoint) then
    DrawRect(rect, { 0.5,1.0,0.5,0.1 })
    DrawLineRect(rect, { 1, 0, 0, 0.1 },borderSize+2)
  elseif (options.selected)and(not options.pressed) then
    DrawRect(rect, { 1, 1, 1, 0.1 })
    DrawLineRect(rect, { 0, 0, 0, 0.1 },borderSize+2)
  else
    DrawLineRect(rect, { 1, 0, 0, 0.1 })
  end
end
...
function widget:DrawScreen()
  ...
  if (#facs>0) then DrawLineRect(facRect, { 1, 1, 1, 0.1 },borderSize+2.5) end
end
