...
local terraCmdTip = {
	[CMD_RAMP] =
		...
		yellow .. '[Modifiers]\n' ..
		'- Hold ' .. green ..
		'Ctrl or Alt' .. white .. ' and ' .. green .. 'drag' .. white .. ' in Step 1 to set start height. \n' ..
	...
	[CMD_LEVEL] = terraformGeneralTip ..
		...
		'Alt' ..
		white ..
		': Snap to starting height / below water level (prevent ships) / below water level (prevent land units). \n' ..
...
local function GetUnitIcon(unitDefID)
	...
	iconTypeCache[unitDefID] = icontypes[(ud and ud.iconType or "default")].bitmap or
		'icons/' .. ud.iconType .. iconFormat
	return iconTypeCache[unitDefID]
end

local function GetCurrentBuildSpeed(unitID, buildSpeed)
	return (Spring.GetUnitCurrentBuildPower(unitID) or 0) * (spGetUnitRulesParam(unitID, "totalBuildPowerChange") or 1) *
		buildSpeed
end
...
local function GetUnitSelectionTooltip(ud, unitDefID, unitID)
	...
	if not unitSelectionTooltipCache[unitDefID] then
		unitSelectionTooltipCache[unitDefID] = GetHumanName(ud, unitID) ..
			" - " .. GetDescription(ud, unitID) .. selectionTooltip
	end
	...
	if unitID and (not select(3, spGetUnitIsStunned(unitID))) then
		...
		if regen_timer and ud then
			if ((ud.idleTime <= 600) and (regen_timer > 0)) then
				...
				local regenMult = (1 - (spGetUnitRulesParam(unitID, "slowState") or 0)) *
					(1 - (spGetUnitRulesParam(unitID, "disarmed") or 0))
				if regenMult == 0 then
					return
				end
				...
				if ud.customParams.amph_regen then
					...
					if (h < 0) then
						regen = regen +
							math.min(ud.customParams.amph_regen,
								ud.customParams.amph_regen * (-h / ud.customParams.amph_submerged_at))
					end
				end
...
local function GetUnitShieldRegenString(unitID, ud)
	...
	local mult = (spGetUnitRulesParam(unitID, "totalReloadSpeedChange") or 1) *
		(1 - (spGetUnitRulesParam(unitID, "shieldChargeDisabled") or 0))
	if mult == 0 then
		return ""
	end
...
local function GetExtraBuildTooltipAndHealthOverride(unitDefID, mousePlaceX, mousePlaceY, forceUpdate)
			...
			if pos and pos[1] and pos[3] then
				...
				if y then
					if y <= tidalHeight then
						...
						local minWindIncome = mult * energyMult *
							(windMin + (windMax - windMin) * math.max(0, math.min(windMinBound, windGroundSlope * (y - windGroundMin))))
						extraText = ", " ..
							WG.Translate("interface", "wind_range") ..
							" " .. math.round(minWindIncome, 1) .. " - " .. math.round(windMax * mult * energyMult, 1)
	...
	if metalOD and metalOD > 0 and energyOD and energyOD > 0 then
		...
		return extraText ..
			...
			healthOverride, minWind
	end
	return
		extraText ..
		"\n" .. WG.Translate("interface", "od_payback") .. ": " .. WG.Translate("interface", "more energy required"),
		healthOverride, minWind
end
	...
	local function UpdateUnitInfo()
		if unitID then
			...
			if jumpReloadTime then
				jumpBar = UpdateManualFireReload(jumpBar, unitImage, unitID, false, JUMP_RELOAD_PARAM, jumpReloadTime,
					true)
			elseif jumpBar then
...
local function GetSelectionStatsDisplay(parentControl)
	...
	local function UpdateDynamicGroupInfo()
		...
		local unitInfoString = WG.Translate("interface", "selected_units") ..
			": " .. Format(total_count, false, 100) .. "\n" ..
			WG.Translate("interface", "value") ..
			": " .. Format(total_cost, false, 100) .. " / " .. Format(total_finishedcost, false, 100) .. "\n" ..
			WG.Translate("interface", "health") ..
			": " .. Format(total_hp, false, 100) .. " / " .. Format(total_maxhp, false, 100) .. "\n"

		if total_maxShield ~= 0 then
			unitInfoString = unitInfoString ..
				WG.Translate("interface", "shields") ..
				": " .. Format(total_shield, false, 100) .. " / " .. Format(total_maxShield, false, 100) .. "\n"
		end
		if total_totalbp ~= 0 then
			unitInfoString = unitInfoString ..
				WG.Translate("interface", "buildpower") ..
				": " .. Format(total_usedbp, false, 100) .. " / " .. Format(total_totalbp, false, 100) .. "\n"
		end
		if total_metalincome ~= 0 or total_metaldrain ~= 0 or total_energyincome ~= 0 or total_energydrain ~= 0 then
			unitInfoString = unitInfoString ..
				WG.Translate("interface", "metal") ..
				": " ..
				FormatPlusMinus(total_metalincome) ..
				white .. " / " .. FormatPlusMinus(-total_metaldrain) .. white .. "\n" ..
				WG.Translate("interface", "energy") ..
				": " ..
				FormatPlusMinus(total_energyincome) ..
				white .. " / " .. FormatPlusMinus(-total_energydrain) .. white .. "\n"
		end
		if burstClass and total_totalburst ~= 0 then
			unitInfoString = unitInfoString ..
				WG.Translate("interface", "burst_damage") ..
				": " .. ((unreliableBurst and "~") or "") .. Format(total_totalburst, false, 100) .. "\n"
		end
	...
	local function UpdateStaticGroupInfo()
		...
		for i = 1, total_count do
			...
			if unitDefID and not filterUnitDefIDs[unitDefID] then
				...
				total_maxShield = total_maxShield +
					(maxShield[unitDefID] or 0) * (Spring.GetUnitRulesParam(unitID, "totalShieldMaxMult") or 1)
				...
				if burstData and burstClass then
					...
					if burstClass == burstData.class then
						total_totalburst = total_totalburst +
							burstData.damage * (Spring.GetUnitRulesParam(unitID, "projectilesMult") or 1)
...
local function GetSingleUnitInfoPanel(parentControl, isTooltipVersion)
	...
	local costInfoUpdate = GetImageWithText(leftPanel, "costInfoUpdate", PIC_HEIGHT + 4, IMAGE.COST, nil, nil, ICON_SIZE,
		4)
	local metalInfoUpdate = GetImageWithText(leftPanel, "metalInfoUpdate", PIC_HEIGHT + LEFT_SPACE + 4, IMAGE.METAL, nil,
		nil, ICON_SIZE, 4)
	local energyInfoUpdate = GetImageWithText(leftPanel, "energyInfoUpdate", PIC_HEIGHT + 2 * LEFT_SPACE + 4,
		IMAGE.ENERGY, nil, nil, ICON_SIZE, 4)
	local maxHealthLabel = GetImageWithText(rightPanel, "maxHealthLabel", PIC_HEIGHT + 4, IMAGE.HEALTH, nil, NAME_FONT,
		ICON_SIZE, 2, 2)

	local minWindLabel = GetImageWithText(leftPanel, "minWindLabel", PIC_HEIGHT + LEFT_SPACE + 4, IMAGE.WIND_SPEED, nil,
		nil, ICON_SIZE, 4)
	local healthBarUpdate = GetBarWithImage(rightPanel, "healthBarUpdate", PIC_HEIGHT + 4, IMAGE.HEALTH, { 0, 1, 0, 1 },
		GetHealthColor)
		...
		if shieldBarUpdate then
			if ud and (ud.shieldPower > 0 or ud.level) then
				local shieldPower = (spGetUnitRulesParam(unitID, "comm_shield_max") or ud.shieldPower) *
					(Spring.GetUnitRulesParam(unitID, "totalShieldMaxMult") or 1)
				...
				if shieldCurrentPower and shieldPower then
					shieldBarUpdate(true, nil, shieldCurrentPower, shieldPower,
						(shieldCurrentPower < shieldPower) and GetUnitShieldRegenString(unitID, ud))
				end
	...
	local function UpdateDynamicEconInfo(unitDefID, mousePlaceX, mousePlaceY)
		...
		if not (unitID or featureID) then
			extraTooltip, healthOverride, minWind = GetExtraBuildTooltipAndHealthOverride(unitDefID, mousePlaceX,
				mousePlaceY)
		end
		...
		if econStructureDefs[unitDefID].isWind then
			local health = Spring.Utilities.GetUnitMaxHealth and
				Spring.Utilities.GetUnitMaxHealth(unitID, unitDefID, healthOverride) or healthOverride or ud.health
	...
	function externalFunctions.SetDisplay(unitID, unitDefID, featureID, featureDefID, blueprint, morphTime, morphCost,
										  mousePlaceX, mousePlaceY, requiredOnly)
		local teamID
		local ud
...
local function GetTooltipWindow()
	...
	function externalFunctions.SetUnitishTooltip(unitID, unitDefID, featureID, featureDefID, blueprint, morphTime,
												 morphCost, mousePlaceX, mousePlaceY, requiredOnly)
		if unitDefID or featureID or featureDefID then
			unitDisplay.SetDisplay(unitID, unitDefID, featureID, featureDefID, blueprint, morphTime, morphCost,
				mousePlaceX, mousePlaceY, requiredOnly)
			textTooltip:SetVisibility(false)
			...
			if ignoreDelay or (sameObjectIDTime > options.independant_world_tooltip_delay.value) then
				if thingIsUnit then
					...
					if ShowUnitCheck(holdingSpace) then
						tooltipWindow.SetUnitishTooltip(thingID, thingDefID, nil, nil, false, nil, nil, nil, nil,
							requiredOnly)
						return true
					end
				else
					...
					if ShowFeatureCheck(holdingSpace, thingDefID) then
						tooltipWindow.SetUnitishTooltip(nil, nil, thingID, thingDefID, false, nil, nil, nil, nil,
							requiredOnly)
						return true
					end
				end
...
function widget:Initialize()
	...
	local function LanguageUpdate()
		...
		selectionTooltip = "\n" ..
			...
			green .. WG.Translate("interface", "rmb") .. ": " .. WG.Translate("interface", "deselect") .. "\n" ..
			green ..
			"+ " .. WG.Translate("interface", "shift") .. ": " .. WG.Translate("interface", "select_type") .. "\n" ..
			green ..
			"+ " ..
			WG.Translate("interface", "alt") .. ": " .. WG.Translate("interface", "select_type_half") .. "\n" ..
			green ..
			"+ " .. WG.Translate("interface", "ctrl") .. ": " .. WG.Translate("interface", "defer_selection") .. "\n" ..
			green .. WG.Translate("interface", "mmb") .. ": " .. WG.Translate("interface", "go_to") .. "\n" ..
			green .. WG.Translate("interface", "space_click_show_stats")
