...
void main() {
	...
	if (slowed > 0.001) {
		...
		vec4 slowcolor = vec4(0.95, 0.1, 0.95, clamp((baseItensity + 0.35), 0.0, 1.0)) * sqrt(slowed) * 1.6;
		fragColor = mix(slowcolor, fragColor, 0.5 * (1.1 - baseItensity) + (1.0 - slowed) * clamp(effect_level, 0.0, 1.0));
	}
	if (fire) {
		flash = 1.0 - flash;
