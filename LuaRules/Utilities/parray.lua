...
--[[
        ...
        Since it has quite different implementation to usual pqueue ones (binary heap, fibonacci heap) it's called parray,
        which stands for Priority Array. parray has following properties:
                0. It's a (key, value) storage, capable of sorting pairs based on key
                0a. Array is online-sorted. One can supply own comparison function in the class constructor
                ...
                1a. Insert() is fast if inserting value's place is nearby the end of array.
                        For instance for ascending sort order, inserting value bigger than currently present in the parray is very fast (one comparison needed and no swaps)
                ...
                4. FrontTrim() - removing the first element from array (either min or max, depending on the sort order) is ultra fast. It's as fast as incrementing the counter and setting one hash value to nil
        ...
        Key differences from heap-based implementation are:
                ...
                2. Get() and FrontTrim() are faster, because array is stored in sorted form and there is no need to rebalance trees.
        ...
        Use case for this class would be:
                ...
                4. Elements get removed one by one and from front of the array only
...
local parray = setmetatable({}, {__call = function(self,...) return new(self,...) end})
parray.__index = parray

--- Main Functions ---
function parray:Insert(k,v)
...
function parray:Upsert(k, v) --insert or update key with value
...
parray.TrimFront=parray.FrontTrim
...
function parray:ipairs()
        return iter, self.kv, self.kvStart-1
end

function parray:GetIdxs()
