...
local function makeReponsiveDefence(team,unitID,eid,eUnitDefID,aidSearchRange)
	...
	if eud.speed > 0 then
		...
		conByID[unitID].makingDefence = true
		GiveClampedOrderToUnit(unitID, -defenceDef, {x,0,z}, {})
	else
...
local function makeWantedDefence(team,unitID,searchRange, maxDistance, priorityDistance)
	...
	if minDeftID ~= 0 then
		GiveClampedOrderToUnit(unitID, -wantedDefence[minDeftID].ID, {wantedDefence[minDeftID].x,0,wantedDefence[minDeftID].z}, {})
		return true
	else
		return false
...
local function makeAirDefence(team,unitID, searchRange,maxDistance)
	...
	if minID ~= 0 then
		...
		GiveClampedOrderToUnit(unitID, -deID, {bx,0,bz}, {})
...
local function makeMiscBuilding(team, unitID, defId, searchRange, maxRange)
	...
	GiveClampedOrderToUnit(unitID, -defId, {x,0,z}, {})	
	return true
end
...
local function makeRadar(team, unitID, searchRange, minDistance)
	...
	GiveClampedOrderToUnit(unitID, -radarDefID, {x,0,z}, {})	
	return true
end
...
local function makeNano(team,unitID)
	...
	GiveClampedOrderToUnit(unitID, -nanoDefID, {x,0,z}, {})
end
...
local function makeEnergy(team,unitID)
	...
	GiveClampedOrderToUnit(unitID, -energyDefID, {x,0,z}, {})
...
local function setUnitPosting(team, unitID)
	...
	for cid,_ in pairs(a.conJob.mex.con) do
		...
		if total > rand then
			...
			GiveClampedOrderToUnit(unitID, CMD_FIGHT , {x+math.random(-100,100),y,z+math.random(-100,100)}, {})
			break
		end
	end
		...
		if #cQueue == 0 then
			...
			if defData.airFactory then
				...
				if math.random() < a.unitHordingChance then
					...
					GiveClampedOrderToUnit(unitID, CMD_FIGHT, { data.wayX+math.random(-200,200), data.wayY, data.wayZ+math.random(-200,200)}, {})
				end
				local facJob = a.facJob
				local totalImportance = 0
				for i = 1, 8 do
		...
		if data.tempTarget then
			if spValidUnitID(data.tempTarget) and isUnitVisible(data.tempTarget,at.aTeamOnThisTeam) then
				...
				for unitID,_ in pairs(data.unit) do
					if not data.aa[unitID] then
						GiveClampedOrderToUnit(unitID, CMD_FIGHT , {x,y,z}, {})
					end
				end
			else
			...
			if aX == data.aX and aZ == data.aZ then
				...
				if data.disbandable then
					...
					if changed then
						...
						for unitID,_ in pairs(data.unit) do
							if not data.aa[unitID] then
								local x, y, z = spGetUnitPosition(unitID)
								--GiveClampedOrderToUnit(unitID, CMD_FIGHT , {data.aimX ,data.aimY,data.aimZ,}, {})
				...
				if maxX - minX > groupRange or maxZ - minZ > groupRange then
					...
					for unitID,_ in pairs(data.unit) do
						if not data.aa[unitID] then
							GiveClampedOrderToUnit(unitID, CMD_FIGHT , {averageX,gy,averageZ}, {})
						end
					end
				else
					if data.regroup == true then
	...
	for unitID,data in pairs(raiderByID) do
		...
		if (#cQueue == 0 or (#cQueue == 2 and cQueue[1].id == CMD_MOVE)) and data.finished and not retreating then
			...
			elseif tX and not unitInBattleGroupByID[unitID] then
				--GiveClampedOrderToUnit(unitID, CMD_FIGHT , {tX + math.random(-200,200),tY,tZ + math.random(-200,200),}, {})
				for i = 1, battleGroup.count do
	...
	if enemyDefence.count > 0 then
		for unitID,data in pairs(artyByID) do
			...
			if #cQueue == 0 then
				...
				GiveClampedOrderToUnit(unitID, CMD_FIGHT , {enemyDefence[randIndex].x,enemyDefence[randIndex].y, enemyDefence[randIndex].z,}, {})
			end
		end
	end
		...
		for unitID,data in pairs(bomberByID) do
			...
			if #cQueue == 0 then
				...
				if static*2 + mobile < 600 then
					GiveClampedOrderToUnit(unitID, CMD_FIGHT , {enemyOffense[randIndex].x,enemyOffense[randIndex].y, enemyOffense[randIndex].z,}, {})
				end
			end
		end
	end
	...
	for unitID,data in pairs(combatByID) do
		...
		if (#cQueue == 0 or (#cQueue == 2 and cQueue[1].id == CMD_MOVE)) and data.finished and not retreating then
			...
			if eID and not spGetUnitNeutral(eID) then	-- FIXME: remove in 95.0
				spGiveOrderToUnit(unitID, CMD_ATTACK , {eID}, {})
			elseif tX and not unitInBattleGroupByID[unitID] then
				--GiveClampedOrderToUnit(unitID, CMD_FIGHT , {tX + math.random(-300,300),tY,tZ + math.random(-300,300),}, {})
				for i = 1, battleGroup.count do
					if battleGroup[i].unit[unitID] then
	...
	if unScoutedPoint.count > 0 then
		for unitID,data in pairs(scoutByID) do
			...
			if #cQueue == 0 then
				...
				GiveClampedOrderToUnit(unitID, CMD_FIGHT , {unScoutedPoint[randIndex].x,unScoutedPoint[randIndex].y,unScoutedPoint[randIndex].z}, {})
			end
...
local function spotEnemyUnit(allyTeam, unitID, unitDefID,readd)
	...
	if readd or (not at.unitInHeatmap[unitID]) then
		...
		elseif ud.buildSpeed > 0 or ud.isFactory or (ud.customParams.income_energy or ud.energyMake > 0 or ud.customParams.upkeep_energy < 0) or ud.customParams.ismex then -- econ
			...
			if not ud.canFly then
				...
				if ((not jump) or jump == 1) and spGetUnitSeparation(unitID, attackerID, true) < jumpDefs[unitDefID].range + 100 then
					...
					if #cQueue == 0 or cQueue[1].id ~= CMD_JUMP then
						...
						GiveClampedOrderToUnit(unitID, CMD_JUMP, {x+math.random(-30,30),y,z+math.random(-30,30)}, {"alt"} )
					end
				end
			end
		end
	...
	if (aiTeamData[unitTeam]) then
		...
		if (ud ~= nil) and initialiseFaction(unitTeam) and controlledUnit.anyByID[unitID] then
			...
			elseif ud.isBuilding or ud.speed == 0 then -- building
				...
				elseif (ud.customParams.income_energy or ud.energyMake > 0 or ud.customParams.upkeep_energy < 0 or (ud.customParams and ud.customParams.windgen)) then
	...
	if (ud ~= nil) then
		for i,at in pairs(allyTeamData) do
			if at.ai and at.inLOS[unitID] and at.unitInHeatmap[unitID] then --this AI spotted this unit (as enemy) and it got destroyed
				...
				-- if ud.buildSpeed > 0 or ud.isFactory or (ud.energyMake > 0 or ud.customParams.upkeep_energy < 0) or ud.customParams.ismex then -- econ
...
local function ProcessUnitCreated(unitID, unitDefID, unitTeam, builderID, changeAlly)
	...
	if (ud ~= nil) and changeAlly then
		...
		elseif ud.isBuilding or ud.speed == 0 then -- building
			...
			elseif (ud.customParams.income_energy or ud.energyMake > 0 or ud.customParams.upkeep_energy < 0) then
		...
		if (ud ~= nil) and initialiseFaction(unitTeam) and controlledUnit.anyByID[unitID] then
			...
			elseif ud.buildSpeed > 0 then
				if ud.speed > 0 then -- constructor
					...
					GiveClampedOrderToUnit(unitID, CMD_PATROL, { x + 25, y, z - 25 }, {})
