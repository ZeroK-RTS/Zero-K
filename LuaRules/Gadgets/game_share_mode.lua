...
if config.permanentMerge then -- only add these if we're on hivemind mode. These give all the 'hiveminds' special names. Mostly a fun little touch to the game mode.
	...
	Names = {
		long = {
			...
			[3] = 'The <leader> Transconsciousness',
			[4] = 'The <leader> Swarm',
			[5] = 'The <leader> Hivemind',
			[6] = 'Hyron Core <leader>',
			[7] = 'The <leader> Core',
			[8] = 'The <leader> Network',
			[9] = '<leader> Prime Consciousness',
			[10] = 'The <leader> Cult',
		},
		short = {
			[1] = '<leader>',
			[2] = '<leader> Collective',
			[3] = '<leader> Transconsciousness',
			[4] = '<leader> Swarm',
			[5] = '<leader> Hive',
			[6] = 'Hyron Core <leader>',
			[7] = '<leader> Core',
...
local function GetAllyTeamLeader(teamlist)
	...
	for i=1, #teamlist do
		...
		if not AI then
			...
			for p=1, #playerlist do
				...
				if debugMode then spEcho("[Commshare] Stats for " .. spGetPlayerInfo(playerID, false) .. ":\nElo: " .. tostring(elo) .. "\nClan: " .. tostring(clan)) end
				if elo ~= nil then
					elo = tonumber(elo)
				else
					elo = -999999
				end
				if debugMode then spEcho("[Commshare] Player " .. playerID .. "'s Clan: " .. clan) end
				if elo > highestElo then
					if debugMode then spEcho("[Commshare] Highest Elo for team is now " .. elo) end
					highestElo = elo
				...
				if clan ~= "" then
					if clans[clan] == nil then
						...
						if elo > clans[clan].highestelo then
							...
							if debugMode then spEcho("[Commshare] Highest Elo for clan " .. clan .. " is now " .. elo) end
						end
						if clans[clan].players > clanthreshhold then
							if debugMode then spEcho("[Commshare] Clan supremecy detected for " .. clan .. "!") end
							leadername = clan
	...
	if debugMode then spEcho("[Commshare] mergeID: " .. mergeID .. ", leadername: " .. leadername) end
...
local function MergeAllHumans(teamlist,allyID)
	...
	if mergeid == -1 then -- this is a team of AIs
		if debugMode then spEcho("[Commshare] AI allyteam detected. Skipping.") end
		return
	end
	if debugMode then spEcho("[Commshare] MergeID: " .. mergeid) end
	for i = 1, #teamlist do
		local teamID = teamlist[i]
		local _, _, _, AI = spGetTeamInfo(teamlist[i], false)
		if not AI and teamID ~= mergeid then
			...
			if debugMode then spEcho("[Commshare] Skipping team " .. i .. " [inhuman/mergeID]") end
		end
	end
	if debugMode then spEcho("[Commshare] Setting up name for team.") end
	local templateID = math.random(1, Names.templates)
	if debugMode then spEcho("[Commshare] Template ID: " .. templateID) end
	...
	if debugMode then spEcho("[Commshare] Setting allyteam " .. allyID .. "'s name to: " .. longName .. " ( " .. shortName .. " )") end
...
function gadget:RecvLuaMsg(message, playerID) -- Entry points for widgets to interact with the gadget. Also handles PlayerChanged.
	...
	elseif command:find("playerchanged") then -- hack in remerging. this is sent by the gadget's unsynced stuff.
		...
		if not targetSpectator then
			if not playerstates[targetID] then -- this player has commshared or changed state.
				...
				if targetActive ~= playerstates[targetID].active and targetActive and commshareID then -- this player has reconnected.
					...
					if debugMode then spEcho("[Commshare] Remerged " .. targetName) end
				end
		...
		elseif spectator and playerstates[targetID] then -- this player resigned
			if debugMode then spEcho("[Commshare] Disposing of " .. targetName) end
