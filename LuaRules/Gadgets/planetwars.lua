		...
		for _,m in pairs(teamOptions.mobiles) do repeat --hack because there is no continue
			...
			if unitDef.customParams.commtype then
				for _, uid in pairs(Spring.GetTeamUnits(teamnum)) do
					...
					if (defID ~= nil and UnitDefs[defID] ~= nil and UnitDefs[defID].customParams.commtype and UnitDefs[defID].name ~= "chickenbroodqueen") then
...
function gadget:CommandFallback(unitID, unitDefID, teamID, cmdID, cmdParams, cmdOptions)
	...
	if not UnitDefs[unitDefID].customParams.commtype then
