...
local NEAR_REQ = 7 -- How many units have to be in range to swarm (ALL UNITS with the same teamID)
local SNAP_HEIGHT = 50 -- How far up a chicken is placed
		...
		while i <= frame.count do
			...
			if toRemove[unitID] or (not Spring.ValidUnitID(unitID)) then
				...
				if near >= NEAR_REQ then
					...
					Spring.AddUnitImpulse(unitID, 0,10,0)
					Spring.MoveCtrl.Enable(unitID)
					Spring.MoveCtrl.SetPosition(unitID, x, y+SNAP_HEIGHT, z)
					Spring.MoveCtrl.Disable(unitID)
					local xDir = math.cos(dir)
					local zDir = math.sin(dir)
					Spring.AddUnitImpulse(unitID, xDir*HORIZONTAL_IMPULSE,-10+VERTICAL_IMPULSE,zDir*HORIZONTAL_IMPULSE)
				end
				i = i + 1
			end
		end
