...
local function SpawnMeteor()
	...
	if projectileCount >= METEOR_CAPACITY * (Spring.GetUnitRulesParam(unitID, "projectilesMult") or 1) then
...
local function SpawnProjectileThread()
	...
	while true do
		...
		lastSpawnProjectileTime = Spring.GetGameFrame()
		local reloadMult = (stunned_or_inbuild and 0) or (spGetUnitRulesParam(unitID, "lowpower") == 1 and 0) or (GG.att_ReloadChange[unitID] or 1) * (Spring.GetUnitRulesParam(unitID, "projectilesMult") or 1)
...
function script.Create()
	...
	Spring.SetUnitRulesParam(unitID, "meteorsControlled", 0, INLOS_ACCESS)
	spSetUnitRulesParam(unitID, "meteorsControlledMax", METEOR_CAPACITY * (Spring.GetUnitRulesParam(unitID, "projectilesMult") or 1), INLOS_ACCESS)
	local x, _, z = Spring.GetUnitPosition(unitID)
